...
static void M_SetupTouchControlsCustomizationMenu(void);
static void M_HandleTouchControlsCustomization(INT32 x, INT32 y, touchfinger_t *finger, event_t *event);
static boolean M_ExitTouchControlsCustomizationMenu(void);
...
#ifdef TOUCHINPUTS
static void M_DrawTouchControlsCustomization(void);
#endif
...
static menuitem_t OP_TouchControlsMenu[] =
{
	...
	{IT_STRING | IT_CALL, NULL, "Customize...", M_CustomizeTouchControls, 30},
};
...
menu_t OP_TouchCustomizationDef = {
	...
	M_DrawTouchControlsCustomization,
	35, 30,
	0,
	M_ExitTouchControlsCustomizationMenu
};
#endif
...
boolean M_Responder(event_t *ev)
{
	...
	else if (menuactive)
	{
		...
		else if (ev->type == ev_touchdown || ev->type == ev_touchmotion)
		{
			...
			{
				M_HandleTouchControlsCustomization(x, y, finger, ev);
				return true;
			}
...
static void M_CustomizeTouchControls(void)
{
	if (cv_touchpreset.value != touchpreset_none)
		M_StartMessage(M_GetText("You must not have a preset selected\nto access this menu.\n\n" PRESS_A_KEY_MESSAGE), NULL, MM_NOTHING);
	else
	{
		M_SetupTouchControlsCustomizationMenu();
...
//
static boolean M_FingerTouchesRect(INT32 fx, INT32 fy, INT32 rx, INT32 ry, INT32 rw, INT32 rh)
{
...
//
static void M_GetTouchButtonRect(touchconfig_t *btn, INT32 *x, INT32 *y, INT32 *w, INT32 *h, boolean inside)
{
...
#define touchcust_numresizepoints 8
...
//
static void M_GetTouchButtonResizePoint(touchconfig_t *btn, vector2_t *point, INT32 *x, INT32 *y, INT32 *w, INT32 *h)
{
	INT32 tx, ty, tw, th;
	...
	M_GetTouchButtonRect(btn, &tx, &ty, &tw, &th, false);
...
//
// Get the position of an option of a button
//
static void M_GetTouchButtonOptionRect(touchconfig_t *btn, touchcust_buttonstatus_t *btnstatus, touchcust_option_e opt, INT32 *x, INT32 *y, INT32 *w, INT32 *h, UINT8 *col, char *str)
{
	//INT32 bx, by, bw, bh;
	INT32 tx, ty, tw, th;
	...
	M_GetTouchButtonRect(btn, &tx, &ty, &tw, &th, true);
...
//
static boolean M_FingerTouchesButton(INT32 x, INT32 y, touchconfig_t *btn, boolean inside)
{
	INT32 tx, ty, tw, th;

	M_GetTouchButtonRect(btn, &tx, &ty, &tw, &th, inside);

	return M_FingerTouchesRect(x, y, tx, ty, tw, th);
}

//
// Moves a button to (x, y)
//
static void M_MoveTouchCustButtonTo(touchconfig_t *btn, INT32 x, INT32 y)
{
...
//
static void M_OffsetTouchCustButton(touchconfig_t *btn, fixed_t offsx, fixed_t offsy)
{
	btn->x *= BASEVIDWIDTH;
	btn->y *= BASEVIDHEIGHT;
	btn->x += offsx;
...
//
static fixed_t M_RoundSnapCoord(fixed_t a, fixed_t b)
{
...
//
static void M_SnapTouchCustButtonToGrid(touchconfig_t *btn)
{
	...
	btn->x = M_RoundSnapCoord(btn->x, gridx);
	btn->y = M_RoundSnapCoord(btn->y, gridy);

	btn->w = M_RoundSnapCoord(btn->w, sgridx);
	btn->h = M_RoundSnapCoord(btn->h, sgridy);
...
// Adds a button
//
static INT32 M_AddTouchButton(INT32 x, INT32 y, touchfinger_t *finger, event_t *event)
{
	...
	M_MoveTouchCustButtonTo(btn, touchcust_addbutton_x, touchcust_addbutton_y);
...
// Removes a button
//
static void M_RemoveTouchButton(touchconfig_t *btn)
{
	memset(btn, 0x00, sizeof(touchconfig_t));
	btn->hidden = true;
}
...
// Clears a selection
//
static void M_ClearTouchCustSelection(touchcust_buttonstatus_t *selection)
{
...
//
static void M_ClearAllTouchCustSelections(void)
{
	INT32 i;
	for (i = 0; i < num_gamecontrols; i++)
	{
		if (touchcustbuttons[i].selected)
			M_ClearTouchCustSelection(&touchcustbuttons[i]);
	}
}
...
static void M_OpenTouchControlsCustomizationSubmenu(touchcust_submenu_e submenu)
{
	touchcust_submenu = submenu;
	M_ClearAllTouchCustSelections();
}

static void M_CloseTouchControlsCustomizationSubmenu(void)
{
	touchcust_submenu = touchcust_submenu_none;
}

static boolean M_HandleSubmenuButtons(INT32 fx, INT32 fy, touchfinger_t *finger, event_t *event)
{
	...
	switch (event->type)
	{
		case ev_touchdown:
			...
			for (i = 0; i < touchcust_submenu_numbuttons; i++)
			{
				...
				if (M_FingerTouchesRect(fx, fy, bx, by, bw, bh))
				{
					btn->isdown = finger;
...
static void M_DrawSubmenuButtons(void)
{
	INT32 bx, by, bw, bh;
	INT32 i;
...
static void M_GetSubmenuListItems(size_t *t, size_t *i, size_t *b, size_t *height, boolean scrolled)
{
	INT32 sel = 0;
...
static void M_GetTouchCustScrollbar(INT32 basex, INT32 basey, INT32 *x, INT32 *y, INT32 *w, INT32 *h, boolean scrolled)
{
	size_t i, m;
	size_t t, b;

	m = TOUCHCUST_SUBMENU_HEIGHT;
	M_GetSubmenuListItems(&t, &i, &b, &m, scrolled);
...
static void M_DrawTouchCustSubmenuList(INT32 x, INT32 y)
{
	size_t i, m;
	size_t t, b; // top and bottom item #s to draw in directory

	V_DrawFill(x, (y - 16) + (lsheadingheight - 3), TOUCHCUST_SUBMENU_WIDTH, 1, 0);
	V_DrawFill(x, (y - 16) + (lsheadingheight - 2), TOUCHCUST_SUBMENU_WIDTH, 1, 30);

	m = TOUCHCUST_SUBMENU_HEIGHT;
	V_DrawFill(x, y - 1, TOUCHCUST_SUBMENU_WIDTH, m, 159);

	M_GetSubmenuListItems(&t, &i, &b, &m, true);

	// draw the scroll bar
	{
		INT32 sx, sy, sw, sh;
		M_GetTouchCustScrollbar(x, y, &sx, &sy, &sw, &sh, true);
...
static boolean M_IsMovingTouchCustSubmenuScrollbar(void)
{
	INT32 i;
...
static void M_AddTouchButtonFromSubmenu(INT32 x, INT32 y, touchfinger_t *finger, event_t *event)
{
	INT32 gc = M_AddTouchButton(x, y, finger, event);
	touchcust_buttonstatus_t *btnstatus = &touchcustbuttons[gc];

	M_CloseTouchControlsCustomizationSubmenu();
...
static void M_ExitTouchCustSubmenu(INT32 x, INT32 y, touchfinger_t *finger, event_t *event)
{
	(void)x;
	(void)y;
	(void)finger;
...
static void M_TouchCustSubmenu_AddNewButton(INT32 fx, INT32 fy, touchfinger_t *finger, event_t *event)
{
	...
	M_GetSubmenuListItems(&t, &i, &b, &m, true);
	M_GetTouchCustScrollbar(mx, my, &sx, &sy, &sw, &sh, (M_IsMovingTouchCustSubmenuScrollbar() ? false : true));
	...
	if (M_HandleSubmenuButtons(fx, fy, finger, event))
		return;
	...
	switch (event->type)
	{
		case ev_touchdown:
			...
			for (i = t; i <= b; i++)
			{
				...
				if (M_FingerTouchesRect(fx, fy, left, top, mw, mh))
				{
			...
			if (!foundbutton && M_FingerTouchesRect(fx, fy, sx, sy, sw, sh))
			{
...
static void M_TouchCustSubmenuDrawer_AddNewButton(void)
{
	if (curfadevalue)
		V_DrawFadeScreen(0xFF00, curfadevalue);

	M_DrawTouchCustSubmenuList(TOUCHCUST_SUBMENU_BASEX, TOUCHCUST_SUBMENU_BASEY);
	M_DrawSubmenuButtons();
}

static void (*touchcust_submenufuncs[num_touchcust_submenus]) (INT32 x, INT32 y, touchfinger_t *finger, event_t *event) =
{
	NULL,
	M_TouchCustSubmenu_AddNewButton,
};

static void (*touchcust_submenudrawfuncs[num_touchcust_submenus]) (void) =
{
	NULL,
	M_TouchCustSubmenuDrawer_AddNewButton,
};

//
// Handles button resizing
//
static boolean M_HandleResizePointSelection(INT32 x, INT32 y, touchfinger_t *finger, touchconfig_t *btn, touchcust_buttonstatus_t *btnstatus)
{
	INT32 i;
	...
	if (btnstatus->isresizing == touchcust_resizepoint_none)
	{
		for (i = 0; i < touchcust_numresizepoints; i++)
		{
			...
			M_GetTouchButtonResizePoint(btn, point, &px, &py, &pw, &ph);
			if (M_FingerTouchesRect(x, y, px, py, pw, ph))
			{
				finger->x = x;
				finger->y = y;
	...
	else
	{
		...
		switch (corner)
		{
			case touchcust_resizepoint_topleft:
				M_OffsetTouchCustButton(btn, (btn->w > MINBTNWIDTH ? dx : 0), (btn->h > MINBTNHEIGHT ? dy : 0));
				btn->w -= dx;
				btn->h -= dy;
				break;
			case touchcust_resizepoint_topright:
				M_OffsetTouchCustButton(btn, 0, (btn->h > MINBTNHEIGHT ? dy : 0));
				btn->w += dx;
				btn->h -= dy;
				break;

			case touchcust_resizepoint_bottomleft:
				M_OffsetTouchCustButton(btn, (btn->w > MINBTNWIDTH ? dx : 0), 0);
				btn->w -= dx;
				btn->h += dy;
				break;
...
//
static boolean M_IsFingerTouchingButtonOptions(INT32 x, INT32 y, touchfinger_t *finger, touchconfig_t *btn, touchcust_buttonstatus_t *btnstatus)
{
	...
	{
		INT32 px, py, pw, ph;
		M_GetTouchButtonOptionRect(btn, btnstatus, opt, &px, &py, &pw, &ph, NULL, NULL);
		if (M_FingerTouchesRect(x, y, px, py, pw, ph))
			return true;
	}
...
static boolean M_HandleButtonOptions(INT32 x, INT32 y, touchfinger_t *finger, touchconfig_t *btn, touchcust_buttonstatus_t *btnstatus, boolean execute)
{
	INT32 px, py, pw, ph;
		...
		if (M_FingerTouchesRect(x, y, px, py, pw, ph))
		{
			...
			switch (opt)
			{
				case touchcust_option_snaptogrid:
					...
					M_SnapTouchCustButtonToGrid(btn);
					break;
				case touchcust_option_remove:
					M_RemoveTouchButton(btn);
					M_ClearTouchCustSelection(btnstatus);
					break;
				default:
					break;
			}
...
struct {
	...
	//{"Joystick / D-Pad",     gc_dpad},
	{"Jump",                 gc_jump},
	{"Spin",                 gc_use},
#if 0
	{"Look Up",              gc_lookup},
	{"Look Down",            gc_lookdown},
	{"Look Left",            gc_turnleft},
	{"Look Right",           gc_turnright},
#endif
...
static boolean M_SetupTouchCustNewButtonSubmenu(touchfinger_t *finger)
{
	INT32 i;
	...
	btn->color = 112;
	btn->name = "ADD";
	btn->action = M_AddTouchButtonFromSubmenu;
	...
	btn->color = 35;
	btn->name = "EXIT";
	btn->action = M_ExitTouchCustSubmenu;
...
//
static boolean M_HandleTouchCustLongPress(void *f)
{
	touchfinger_t *finger = (touchfinger_t *)f;

	if (finger->longpress >= (TICRATE/2))
	{
		if (!M_SetupTouchCustNewButtonSubmenu(finger))
			return true;

		M_OpenTouchControlsCustomizationSubmenu(touchcust_submenu_newbtn);
		highlightflags = V_YELLOWMAP;
	...
	switch (event->type)
	{
		...
		case ev_touchmotion:
			...
			if (finger->u.gamecontrol != gc_null)
			{
				...
				optionsarea = M_IsFingerTouchingButtonOptions(x, y, finger, btn, btnstatus);
			}
			...
			//for (i = 0; i < num_gamecontrols; i++)
			{
				...
				// Select options
				if (M_HandleButtonOptions(x, y, finger, btn, btnstatus, false) && (i == finger->u.gamecontrol))
				{
				...
				else if (btnstatus->selected && touchmotion && (i == finger->u.gamecontrol))
				{
					...
					{
						M_HandleResizePointSelection(x, y, finger, btn, btnstatus);
						foundbutton = true;
						break;
					}

					if (M_HandleResizePointSelection(x, y, finger, btn, btnstatus) && (!btnstatus->moving))
						btnstatus->resizearea = true;
					else
					{
						fixed_t dx = FixedDiv((x - finger->x) * FRACUNIT, vid.dupx * FRACUNIT);
						fixed_t dy = FixedDiv((y - finger->y) * FRACUNIT, vid.dupy * FRACUNIT);
						M_OffsetTouchCustButton(btn, dx, dy);
				...
				// Check if your finger touches this button.
				else if (M_FingerTouchesButton(x, y, btn, false) && (!optionsarea))
				{
			...
			if (finger->longpressaction && touchmotion)
			{
				...
				finger->longpressaction = M_HandleTouchCustLongPress;
		...
		case ev_touchup:
			...
			if (gc > gc_null)
			{
				...
				// Select options
				if (M_HandleButtonOptions(x, y, finger, btn, btnstatus, true))
				{
...
static void M_DrawTouchGrid(void)
{
	INT32 i;
	INT32 col = 0;
	INT32 scol = 15;
	...
	for (i = 0; i < BASEVIDWIDTH; i += TOUCHGRIDSIZE)
		V_DrawFill(i-1, 0, 1, BASEVIDHEIGHT, col);
	for (i = 0; i < BASEVIDWIDTH; i += TOUCHGRIDSIZE)
		V_DrawFill(0, i-1, BASEVIDWIDTH, 1, col);
}

static void M_DrawTouchButtonOption(touchcust_option_e opt, touchconfig_t *btn, touchcust_buttonstatus_t *btnstatus)
{
	INT32 px, py, pw, ph;
	INT32 sx, sy;
	INT32 strwidth, strheight;
	...
	M_GetTouchButtonOptionRect(btn, btnstatus, opt, &px, &py, &pw, &ph, &col, str);
...
#if 0
	INT32 row = (flash/3)%FADECOLORMAPROWS;
	V_DrawFixedPatch(40<<FRACBITS, 40<<FRACBITS, FRACUNIT/4, 0, W_CachePatchName("LOGCAT", PU_PATCH), &fadecolormap[row*256]);
#endif
	...
	for (i = 0; i < num_gamecontrols; i++)
	{
		...
		if (!btnstatus->moving)
		{
			...
			for (j = (INT32)touchcust_options_first; j < (INT32)num_touchcust_options; j++)
				M_DrawTouchButtonOption(j, btn, btnstatus);
